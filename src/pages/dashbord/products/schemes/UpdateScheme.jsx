...
const UpdateScheme = ({ schemeForUpdate, product }) => {
  ...
  useEffect(() => {
    setFormData({
      qty: schemeForUpdate.scheme.qty,
      unit: schemeForUpdate.scheme.unit,
      message: schemeForUpdate.scheme.message,
      offerPercent: schemeForUpdate.scheme.offerPercent,
      inStock: schemeForUpdate.scheme.inStock,
      price: schemeForUpdate.scheme.price,
      priorityToShow: schemeForUpdate.scheme.priorityToShow,
      timeToDeliver: schemeForUpdate.scheme.timeToDeliver,
      schemeId: schemeForUpdate.scheme._id,
    });
    setImages(schemeForUpdate.scheme.imageUrls);
  }, []);
  // fetch units
  (async () => {
    try {
    ...
    files.forEach((file) => {
      ...
      reader.onload = (loadEvent) => {
        setImages((prevImages) =>
          [...prevImages, loadEvent.target.result].slice(0, 5)
        );
      };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    // filter image which is aready uploaded into cloudinary
    const imageUrls = images.filter((image) =>
      schemeForUpdate.scheme.imageUrls.includes(image)
    );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          ...
          <div className="mb-2">
            ...
            <input
              type="text"
              name="timeToDeliver"
              placeholder="eg. 1 MIN 30 MIN, 1 day..."
